  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-[#0a0a0a] text-white pt-20">
        {/* Header */}
        <div className="pt-12 pb-10 text-center px-5">
          ...
          <h1 className="font-display text-5xl sm:text-6xl md:text-7xl lg:text-8xl tracking-tighter">
            OUR MENU
          </h1>
        ...
        <div className="max-w-4xl mx-auto px-5 sm:px-6 pb-20">
          <motion.div
            className="relative h-[620px] sm:h-[680px] md:h-[720px] perspective-[1800px] mx-auto"
            drag="x"
            ...
            <AnimatePresence mode="wait">
              ...
              >
                <div className="flex h-full flex-col md:flex-row">
                  {/* Left Side - Visual */}
                  <div className="md:w-5/12 bg-black/80 p-6 sm:p-8 md:p-12 flex flex-col items-center justify-center relative">
                    ...
                    <span className="text-6xl md:text-7xl mb-6">{current.emoji}</span>
                    <h2 className="font-display text-4xl sm:text-5xl md:text-6xl text-center tracking-tight text-amber-300 leading-none">
                      {current.title}
                    </h2>
                  </div>

                  {/* Right Side - Menu Items */}
                  <div className="md:w-7/12 p-6 sm:p-8 md:p-12 bg-zinc-950 flex flex-col">
                    <div className="flex-1 space-y-8 pt-2">
                      {current.items.map((item, idx) => (
                        <motion.div
                          ...
                          className="flex justify-between items-start border-b border-white/10 pb-6 last:border-none group"
                        >
                          <div className="pr-6">
                            <h3 className="text-lg sm:text-xl font-medium group-hover:text-amber-400 transition-colors">
                              {item.name}
                            </h3>
                          ...
                          <span className="font-mono text-amber-400 text-xl sm:text-2xl font-medium whitespace-nowrap pt-1">
                            {item.price}
                          </span>
                        </motion.div>
                      ))}
                ...
                {/* Page Curl Effect */}
                <div className="absolute bottom-0 right-0 w-28 h-28 bg-gradient-to-br from-amber-400/10 to-transparent pointer-events-none" />
              </motion.div>
            </AnimatePresence>
          </motion.div>

          {/* Navigation Controls */}
          <div className="flex justify-center items-center gap-6 sm:gap-8 mt-12">
            <button
              onClick={prevPage}
              className="flex items-center gap-2 px-5 sm:px-6 py-3.5 rounded-full border border-amber-400/30 hover:border-amber-400 hover:bg-white/5 transition-all text-sm sm:text-base"
            >
        ...
        <p className="text-center text-zinc-500 text-sm pb-12">
          Drag / Swipe left or right to flip pages • All prices in ZAR
        </p>
      </div>
    </>
  );
