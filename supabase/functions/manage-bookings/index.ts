  ...
  try {
    ...
    if (action === "create") {
      ...
      if (occupancy) {
        await supabase
          .from("occupancy_tracking")
          .update({ current_occupancy: occupancy.current_occupancy + guests })
          .eq("id", occupancy.id);
      } else {
        await supabase
          .from("occupancy_tracking")
      ...
      // Send professional booking confirmation
      await fetch(`${env.SUPABASE_URL}/functions/v1/send-booking-notification`, {
        method: "POST",
        headers: {
    ...
    if (action === "update-status") {
      if (!booking_id || !status) throw new Error("booking_id and status required");
